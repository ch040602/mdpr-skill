...
The pipeline image is generated from a one-slide PowerPoint deck at `docs/assets/pipeline-overview.pptx` and exported as a high-resolution PNG through Microsoft PowerPoint. It separates the flow into large responsibility regions, uses the LLM reasoning result only as dashed hint input, and uses arrow weight and type to distinguish primary flow, optional reasoning support, deterministic rule links, and validation flow.
