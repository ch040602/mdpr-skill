...
| 41 | The Examples evidence card sampled three different source families but displayed the basename-only label `deck.md` three times, collapsing distinct evidence into one visible identifier. | Compute the shortest unique source-relative suffix only within colliding basename groups. Preserve unique basenames, source order, editable text, and reject absolute, parent-relative, or irreducibly duplicate paths. | Red retained three identical artifact labels after the helper unit controls passed. The RGB-rendered green card shows `basic/deck.md`, `comparison/deck.md`, and `pipeline/deck.md`; Docs, Root, ADR, slide count, font floor, overflow, surface history, and report status remain unchanged. |
