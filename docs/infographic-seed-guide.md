...
PPT BIZCAM references are reviewed for design method vocabulary rather than copied assets. The latest recursive pass scans numbered post pages from `https://pptbizcam.co.kr/?p=11677`, downloads 50 public PPT files into `.cache/pptbizcam`, renders 984 PNG slide images through Microsoft PowerPoint, analyzes 100 PNG samples, and derives 60 reusable object/decorator rules. Relevant recurring methods include simple shape systems, table grid systems, small icon markers, document/notebook metaphors, browser/window frames, arc/ring charts, line graphs used as background, gauge-like charts, connected chart strips, target-ring frames, pictorial metaphor charts, timelines, hub/spoke diagrams, and equal-geometry quadrant layouts. See `docs/pptbizcam-pattern-taxonomy.md`.
